...
```{python}
df_type_filter["full_dict"] = df_type_filter.to_dict(orient="records")
df_type_filter
```

```{python}
def paper_review(df_type_filter):
    prompt = f"Considering the following data on this article {df_type_filter}, provide a summary review for each article my report."

    return llm_chat(prompt)

paper_review_vec = np.vectorize(paper_review)
```

```{python}
df_test = df_type_filter[["title", "abstract", "Problem Statement", "Methods Used", "Research Gap", "Results", "Limitations", "Objectives"]].tail()
df_test["full_dict"] = df_test.to_dict(orient="records")
df_test["llm_summary"] = paper_review(df_test["full_dict"])
# df_test[["Methods Used", "Results"]]
```

```{python}
df_test.to_csv('papers_review.csv', index=False)
```
...
scopus1["llm_summary"] = paper_review(scopus1["full_dict"])
```
